...
class ParetoFrontier {
  public:
    ...
    void remove_dominated() { remove_empty(); }

    // Get number of solutions
    int get_num_sols() const { return sols.size() / NOBJS; }

    // Clear pareto frontier
    void clear() { sols.resize(0); }
...
class ParetoFrontierManager {
  public:
    ...
    ParetoFrontierManager(int size) { frontiers.reserve(size); }
    ...
    void deallocate(ParetoFrontier *frontier) { frontiers.push_back(frontier); }
...
//
template <int N> inline bool AdominatedB(const ObjType *v1, const ObjType *v2) {
    return (v1[N - 1] <= v2[N - 1]) && AdominatedB<N - 1>(v1, v2);
}
template <> inline bool AdominatedB<0>(const ObjType *v1, const ObjType *v2) { return true; }

//
// Check if solution v1 dominates v2
//
template <int N> inline bool AdominatesB(const ObjType *v1, const ObjType *v2) {
    return (v1[N - 1] >= v2[N - 1]) && AdominatesB<N - 1>(v1, v2);
}
template <> inline bool AdominatesB<0>(const ObjType *v1, const ObjType *v2) { return true; }
    ...
    for (int j = 0; j < frontier.sols.size(); j += NOBJS) {
        ...
        for (int i = 0; i < end; i += NOBJS) {
            ...
            if (dominated) {
                ...
                if (must_add) {
                    // solution has not been added - just replace current iterate
                    std::copy(frontier.sols.begin() + j, frontier.sols.begin() + j + NOBJS,
                              &sols[i]);
                    must_add = false;
                } else {
                    // if already added, mark array as "to erase"
