...
// Set arc length
//
inline void MDDArc::set_length(const int _length) { length = _length; }
...
// MDD constructor
//
inline MDD::MDD(const int _num_layers) : num_layers(_num_layers) { layers.resize(num_layers); }
...
// Get MDD width
//
inline int MDD::get_width() const { return width; }

//
// Get max arc value in any layer
//
inline int MDD::get_max_value() const { return max_value; }
...
inline void MDD::print() const {
    ...
    for (int l = 0; l < num_layers; ++l) {
        cout << "\tLayer " << l << endl;
        for (vector<MDDNode *>::const_iterator it = layers[l].begin(); it != layers[l].end();
             ++it) {
            MDDNode *node = *it;
