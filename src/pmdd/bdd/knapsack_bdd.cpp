...
void KnapsackBDDConstructor::update_node_weights(BDD *bdd) {
    ...
    for (int l = 1; l < bdd->num_layers; ++l) {
        // iterate on layers
        for (vector<Node *>::iterator it = bdd->layers[l].begin(); it != bdd->layers[l].end();
             ++it) {
            (*it)->min_weight = INT_MAX;

            // iterate over the incoming zero arcs
            for (vector<Node *>::iterator it_prev = (*it)->prev[0].begin();
                 it_prev != (*it)->prev[0].end(); ++it_prev) {
                (*it)->min_weight = min((*it)->min_weight, (*it_prev)->min_weight);
            }

            // iterate over the incoming one arcs
            for (vector<Node *>::iterator it_prev = (*it)->prev[1].begin();
                 it_prev != (*it)->prev[1].end(); ++it_prev) {
                (*it)->min_weight =
                    min((*it)->min_weight, (*it_prev)->min_weight + inst->coeffs[0][l - 1]);
            }
        }
    }
}
