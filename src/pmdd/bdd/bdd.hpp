...
struct Node {
    ...
    // Set arc weights
    void set_arc_weights(const int type, ObjType *arc_weights) { weights[type] = arc_weights; }
};
...
struct BDD {
    ...
    // Constructor
    BDD(int _num_layers) : num_layers(_num_layers) { layers.resize(num_layers); }
    ...
    // Return terminal
    Node *get_terminal() const { return layers[num_layers - 1][0]; }

    // Return root
    Node *get_root() const { return layers[0][0]; }
};

//
// Node constructor
//
inline Node::Node(const int _layer, int _index) : layer(_layer), index(_index) {
        ...
        if (node->arcs[arc_type] != NULL) {
            for (int i = 0; i < node->arcs[arc_type]->prev[arc_type].size(); ++i) {
                if (node->arcs[arc_type]->prev[arc_type][i] == node) {
                    node->arcs[arc_type]->prev[arc_type][i] =
                        node->arcs[arc_type]->prev[arc_type].back();
    ...
    for (int l = num_layers - 2; l >= 0; --l) {
        for (vector<Node *>::iterator it = layers[l].begin(); it != layers[l].end(); ++it) {
            ...
            if (node->arcs[0] != NULL && node->arcs[1] != NULL) {
                node->status =
                    ((node->arcs[0]->status == 'F') && (node->arcs[1]->status == 'F') ? 'F' : 'T');
