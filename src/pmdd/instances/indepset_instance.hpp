...
struct Graph {
    ...
    Graph *decompose_into_lifted_cliques_k(vector<Graph *> &cliques,
                                           vector<vector<int>> &vertex_ind, int k);

    /** Decompose graphs into lifted cliques */
    Graph *decompose_into_lifted_cliques(vector<Graph *> &cliques, vector<vector<int>> &vertex_ind);
 ...
 * Constructor with number of vertices
 **/
inline Graph::Graph(int num_vertices) : n_vertices(num_vertices), n_edges(0), weights(NULL) {
