...
Graph::Graph(Graph *graph, vector<int> &mapping)
    ...
    for (int i = 0; i < graph->n_vertices; ++i) {
        for (vector<int>::iterator it = graph->adj_list[i].begin(); it != graph->adj_list[i].end();
             it++) {
...
void union_find(int i, int j) { parent[find(i)] = find(j); }
...
Graph *Graph::decompose_into_cluster_cliques(vector<Graph *> &cliques,
    ...
    vertex_ind.erase(std::remove_if(vertex_ind.begin(), vertex_ind.end(),
...
//
void Graph::decompose_into_trees(vector<Graph *> &trees, vector<vector<int>> &vertex_ind) {
