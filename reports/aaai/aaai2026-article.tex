...
% \begin{algorithm}[t!]
% \caption{Bidirectional PF Enumeration and Serial CPU Backend}
% \label{alg:bidir_pf}
% \begin{algorithmic}[1]
% \Require Exact decision diagram $\mathcal{D}$
...
%     \If{$d=\downarrow$}
%         \For{\textbf{each} arc $a=(u,v)$ with $v\in\mathcal{L}_j$}
%             \State
%             $\mathcal{Y}(v)
%             \gets
%             \mathcal{Y}(v)
...
%         \EndFor
%     \Else
%         \For{\textbf{each} arc $a=(u,v)$ with $u\in\mathcal{L}_j$}
%             \State
%             $\mathcal{Y}(u)
%             \gets
%             \mathcal{Y}(u)
...
% \Procedure{BidirectionalPF}{$\mathcal{D},\mathsf{B}$}
%     \State
...
% \State \Return
% \Call{BidirectionalPF}{$\mathcal{D},\mathrm{CPU}$}
% \end{algorithmic}
% \end{algorithm}
...
During enumeration, each DD node stores a \emph{node frontier}, a nondominated set of objective vectors associated with the partial paths through that node. We write $\mathcal{Y}^{\downarrow}(u)$ for the node frontier of the partial paths from the root $\mathbf{r}$ to node $u$, and $\mathcal{Y}^{\uparrow}(u)$ for the node frontier of the partial paths from $u$ to the terminal $\mathbf{t}$. An individual objective vector in a node frontier is a \emph{label}, and a newly generated label that has not yet been filtered is a \emph{candidate label}. During cutset merging we maintain a \emph{global frontier}, a running nondominated set over complete root-to-terminal labels, which results in exact PF $\mathcal{Y}_N$, after all cutset node frontier have been merged. 

Bidirectional PF enumeration extracts $\mathcal{Y}_N$ from an exact decision diagram $\mathcal{D}$ by interleaving root-side and terminal-side layer expansion (\Cref{alg:bidir_pf}). Interleaving keeps the intermediate node frontiers smaller than expansion from either end alone. A root-side layer pointer $i_{\text{td}}$ starts at layer 1 and a terminal-side pointer $i_{\text{bu}}$ starts at layer $n+1$. Each step expands one side, advancing the root-side node frontiers $\mathcal{Y}^{\downarrow}$ to layer $i_{\text{td}}+1$ or the terminal-side node frontiers $\mathcal{Y}^{\uparrow}$ to layer $i_{\text{bu}}-1$. The two directions meet at the \emph{cutset} layer $c$, where $i_{\text{td}} = i_{\text{bu}}$. For each node $u \in \mathcal{L}_c$, the algorithm forms the pairwise sums of the labels in its root-side and terminal-side node frontiers (\Cref{alg:bidir_pf}, line 35). Merging these sets into the global frontier and discarding the dominated labels yields $\mathcal{Y}_N$.
