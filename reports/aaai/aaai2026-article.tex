...
This section describes how we parallelize exact Pareto frontier enumeration on a decision diagram (DD). We use the top-down and coupled enumeration schemes described in \Cref{alg:top_down_pf,alg:coupled_pf}; in particular, the dynamic choice between top-down and bottom-up expansion in the coupled scheme follows the serial enumeration method. Our contribution is the parallel execution of the expensive primitives that arise inside these schemes: candidate generation, dominance filtering, cutset coupling, and frontier merging.

\subsection{Parallel Work Primitives}
\label{sec:parallel_primitives_rewrite}

The enumeration algorithms maintain partial Pareto frontiers at DD nodes. In a top-down expansion, labels are propagated from layer $\mathcal{L}_j$ to $\mathcal{L}_{j+1}$ along outgoing arcs. In a bottom-up expansion, labels are propagated in the reverse direction. In both cases, expanding a layer has two phases. First, candidate labels are generated by shifting an existing node frontier by an arc weight. Second, candidates assigned to the same destination node are filtered by Pareto dominance.

These phases expose several forms of parallelism. The coarsest is \emph{node-level parallelism}: different nodes in the same layer can be processed independently once the previous layer has been finalized. This is attractive on CPUs because it gives each worker ownership of a node frontier and avoids synchronization on shared label sets. A finer granularity is \emph{arc- and label-level parallelism}: each candidate produced by an arc-frontier pair can be materialized independently, provided that the implementation assigns it a disjoint output location. The dominance test exposes still finer \emph{comparison-level parallelism}, since each candidate can be compared with many other candidates in the same node-local set.

The coupled algorithm adds a fourth primitive at the cutset layer. Once the top-down and bottom-up frontiers meet at layer $\mathcal{L}_c$, each cutset node $u$ contributes the product set
\[
...
\]
Thus, cutset coupling contains both product generation and global frontier merging. The product generation is naturally parallel over cutset nodes and over pairs of labels within a node. The merge is a reduction over nondominated sets: partial product frontiers must be combined while discarding points dominated by already retained points. This primitive is distinct from layer expansion, and should not be described solely as assigning one or many threads to each DD node.

\subsection{CPU Implementation}
\label{sec:cpu_impl_rewrite}

The CPU implementation uses node-level parallelism for layer expansion. For a selected layer, the outer loop over target nodes is parallelized with dynamic scheduling. Each worker receives a node, scans the relevant incoming or outgoing arcs, generates the shifted labels, and maintains the node's local Pareto frontier. Since only one worker mutates a given node frontier, no lock is needed during the merge and dominance-filtering operations for that node.

This design is simple and effective when work is reasonably spread across the nodes of a layer. It also handles irregular layers better than static scheduling because workers that finish small frontiers can take additional nodes. Its limitation is that a single very large node frontier remains serial within one worker. The CPU implementation does not split the dominance filtering of one node across many workers; doing so would require either synchronization on the node frontier or an additional materialization and reduction scheme.

For coupled enumeration, the CPU also parallelizes the cutset. Each worker processes a subset of cutset nodes and builds a private partial frontier from the products $\mathcal{Y}^{\downarrow}(u) \oplus \mathcal{Y}^{\uparrow}(u)$. These private frontiers are then merged by a pairwise tree reduction until one frontier remains. This reduction exposes parallelism in the global merge while preserving thread-local frontier updates within each partial merge.

\subsection{GPU Implementation}
\label{sec:gpu_impl_rewrite}

The GPU implementation uses finer-grained parallelism because node-level work alone is too irregular for throughput-oriented hardware. Before enumeration, each DD layer is flattened into contiguous arrays describing arc endpoints, arc weights, and per-node arc offsets. This representation is analogous to a compressed sparse row layout and allows the same kernels to operate on BDD and MDD layers.

Each GPU layer expansion is organized as four passes. First, a counting pass computes how many candidates each arc will generate from the current source frontier. An exclusive prefix sum converts these counts into disjoint offsets in a flat candidate buffer. Second, candidate generation writes shifted labels into this buffer in parallel. Since every arc has a precomputed output range, the writes are race-free. Third, dominance filtering is performed within each destination-node segment of the candidate buffer. Large segments receive multiple thread blocks, and each block compares a tile of candidates using registers and shared memory to reduce repeated global-memory reads. Fourth, a compaction pass uses survivor flags and a prefix sum to write the nondominated labels into the next layer's frontier representation.

This layout also provides load balancing. Nodes with few candidates receive little GPU work, while nodes with many candidates receive more blocks during dominance filtering. When a layer would produce too many candidates at once, the implementation processes node ranges in memory-bounded batches. The same expansion and filtering passes are applied to each batch, and the survivors are appended to the next frontier representation.

The GPU cutset step follows the same principle but uses a different primitive. Active cutset nodes are those with nonempty top-down and bottom-up frontiers. These nodes are sorted by a size score and partitioned into batches whose Cartesian-product size is bounded. For each batch, a kernel materializes all sums $\mathbf{y}^{\downarrow}+\mathbf{y}^{\uparrow}$ for the nodes in the batch. The batch is then filtered against the current global frontier, self-pruned by dominance filtering, and merged into the global frontier by mutually removing dominated points. Thus, unlike the CPU implementation, the GPU cutset merge is implemented as a batched running-frontier merge rather than a tree reduction over thread-local partial frontiers.

The top-down GPU variant is obtained by removing the bottom-up state and the cutset-coupling step. It repeatedly applies the top-down layer-expansion passes until the terminal layer is reached, at which point the terminal frontier is the complete Pareto frontier.
