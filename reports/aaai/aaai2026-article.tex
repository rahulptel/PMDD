...
\begin{enumerate}
    \item How does the best serial multiobjective DDs (\textsc{Smdd}) compare with the state-of-the-art MIP-based defining point algorithm (DPA)?
    \item What speedup does the CPU variant of \textsc{Pmdd} (\textsc{PmddC}) achieve over \textsc{Smdd}?
    \item Can GPU-based \textsc{Pmdd} (\textsc{PmddG}) outperform \textsc{PmddC}?
\end{enumerate}
