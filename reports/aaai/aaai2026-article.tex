...
In this work, we identify the serial operations amenable to parallelization and develop efficient parallel implementations to accelerate enumeration on both CPU and GPU.
...
Many discrete optimization problems that arise in practice are inherently multiobjective. In vehicle routing, for example, a dispatcher does not just minimize routing cost. Service quality also depends on how consistently each customer is visited by the same driver and at a similar time each day, since both build trust and route familiarity \cite{kovacs2015multi}. These goals conflict, since restricting driver assignments or tightening arrival-time windows typically forces costlier routes, so no single routing plan is best on cost, driver consistency, and arrival-time consistency at once. Such plans cannot be ranked by a single score; they are compared through Pareto dominance, where one solution dominates another if it matches it on every objective and improves on at least one. The goal of exact multiobjective optimization is to enumerate every nondominated solution, since each one represents a distinct trade-off a decision-maker might reasonably prefer over the rest. This complete collection is known as the \emph{Pareto frontier} (PF). It exposes every trade-off among the objectives without approximation and leaves the final choice to the decision-maker.
...
\begin{itemize}
    \item We identify where bidirectional PF enumeration decomposes for parallel execution: across nodes within a layer, across labels within a node, and across products and merges in the cutset phase.
    \item We give a CPU implementation that dynamically schedules node expansion over OpenMP threads and reduces cutset frontiers with a lock-free pairwise tree merge.
    \item We give a GPU implementation that flattens the diagram into offset-indexed arc arrays, expands each layer with a four-kernel pipeline, and merges the cutset in memory-bounded batches against a running frontier.
