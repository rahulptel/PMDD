...
The opportunities for parallelism are most easily seen directly from Algorithm~\ref{alg:coupled_pf}. In the root-side expansion, the loop over nodes in line~5 can be parallelized because the label set of each destination node is updated independently once the previous layer has been finalized. The same observation applies to the terminal-side expansion in line~14. The work performed inside a node consists of scanning adjacent arcs, generating shifted labels, merging them into the node-local set, and applying the dominance filter in lines~7 and~17.

There is a second, finer level of parallelism inside lines~7 and~17. The candidates generated for a node can be materialized independently, and the pairwise dominance comparisons within that node-local candidate set can also be distributed. This finer decomposition requires an explicit candidate buffer and a careful layout so that different workers write to disjoint locations. It is therefore a different implementation choice from assigning one worker to own an entire node frontier.
...
\]
These products are independent across cutset nodes and across pairs of labels within a node. The final merge in lines~34--36 is different: it is a reduction over nondominated sets. Partial frontiers produced at the cutset must be combined while discarding points dominated by already retained points.
...
Each CPU worker owns the frontier of the node it is assigned. It scans the relevant arcs, generates shifted labels, merges them into the local frontier, and applies the dominance filter corresponding to lines~7 and~17. Since no other worker mutates that node frontier, the implementation avoids locks. The limitation is that a single large node remains serial within one worker; the CPU implementation does not split the dominance filtering of one node across multiple workers.

The CPU also parallelizes the cutset products in lines~31--33. Each worker processes a subset of cutset nodes and builds a private partial frontier from the products $\mathcal{Y}^{\downarrow}(u) \oplus \mathcal{Y}^{\uparrow}(u)$. The final merge in lines~34--36 is implemented as the pairwise tree reduction shown in \Cref{alg:cpu_tree_reduction}. This exposes parallelism in the global merge while keeping each individual frontier update thread-local.

\subsection{GPU Implementation}
\label{sec:gpu_impl_rewrite}

The GPU implementation follows the same enumeration logic as Algorithm~\ref{alg:coupled_pf}, but changes the unit of work. Rather than assigning one worker to own a node frontier, it materializes candidates in flat buffers and then applies data-parallel filtering and compaction kernels. The high-level procedure is shown in \Cref{alg:gpu_coupled_pf}: the DD is first flattened, each selected layer expansion invokes the same four-kernel pipeline, and the cutset phase is handled by a separate batched coupling routine.
...
\]
Here $\kappa_a$ is the number of candidates generated by arc $a$, $\phi^c_a$ is the starting offset of arc $a$ in the flat candidate buffer, and $\Gamma_v$ is the total number of candidates assigned to destination node $v$. The buffer $\beta$ stores the generated labels, and the binary vector $\sigma$ marks the labels that survive node-local dominance filtering.

First, \Cref{alg:compute_arc_offsets} counts how many candidates each arc will produce and uses a prefix sum to assign disjoint output segments in $\beta$. Second, \Cref{alg:expand_candidates} fills this buffer with shifted labels. The prefix-sum offsets are the key invariant: candidates generated by different arcs write to disjoint memory, but candidates for the same destination node occupy one contiguous segment. Third, \Cref{alg:dynamic_1D_kernel} filters each destination-node segment using a load-balanced grid of thread blocks; nodes with larger candidate sets receive more blocks. Fourth, \Cref{alg:compact_survivors} compacts the surviving labels into the next frontier representation. These four steps implement the parallelization of the local generation, merge, and dominance filtering in lines~7 and~17 of Algorithm~\ref{alg:coupled_pf}.
...
The cutset phase uses a different schedule, shown in \Cref{alg:gpu_cutset_coupling}. Active cutset nodes are those with nonempty root-side and terminal-side frontiers. They are sorted by frontier size and processed in product-bounded batches so that the Cartesian product for a batch fits in memory. For each batch, \Cref{alg:materialize_cutset_products} materializes all sums $\mathbf{y}^{\downarrow}+\mathbf{y}^{\uparrow}$ corresponding to lines~31--33 of Algorithm~\ref{alg:coupled_pf}. The batch is then filtered against the current global frontier, self-pruned, and merged into the running frontier. Thus the GPU does not use the CPU tree reduction in \Cref{alg:cpu_tree_reduction}; it uses a batched running-frontier merge, where batching is both a load-balancing mechanism and a memory-control mechanism.
...
\begin{algorithmic}[1]
\Require Private partial frontiers $\mathcal{P} = [\mathcal{P}_1,\dots,\mathcal{P}_q]$
...
A one-sided enumeration variant fixes the terminal-side layer pointer at $\mathcal{L}_{n+1}$ and expands only from the root side, as shown in \Cref{alg:top_down_pf}. The cutset is then the terminal layer, and the terminal label set is the Pareto frontier.
