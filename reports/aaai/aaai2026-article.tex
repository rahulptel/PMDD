...
\begin{abstract}
Decision diagrams (DDs) can efficiently enumerate the exact Pareto frontier (PF) for multiobjective discrete optimization problems with a compact dynamic programming formulation.
However, existing DD-based PF enumeration algorithms are serial, presenting significant opportunities for parallelization.
In this work, we first identify the core serial operations amenable to parallelization, such as layer expansion, candidate-label generation, dominance filtering, cutset-product materialization, and frontier merging.
We then develop efficient parallel implementations of these operations, tailored to CPU and GPU architectures, to accelerate enumeration.
Finally, we evaluate our methods on benchmark instances of the multiobjective knapsack, set packing, and traveling salesperson problems with up to seven objectives, comparing against a state-of-the-art MIP approach and the state-of-the-art serial DD implementation.
DD-based enumeration proves more robust than the MIP baseline on MOKP and MOTSP, though the MIP baseline remains competitive on some lower-objective MOSPP instances.
Relative to serial DD enumeration, our parallel CPU variants achieve speedups of up to $7.83\times$, while GPU variants reach up to $20.88\times$ when intermediate frontiers fit in GPU memory.
Code is available at \url{https://anonymous.4open.science/r/DD-C683/}.
\end{abstract}
