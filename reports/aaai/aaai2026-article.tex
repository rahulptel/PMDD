...
The enumeration maintains partial Pareto frontiers at DD nodes from both sides of the diagram. A root-side expansion propagates labels from layer $\mathcal{L}_j$ to $\mathcal{L}_{j+1}$ along outgoing arcs. A terminal-side expansion propagates labels in the reverse direction. In both cases, expanding a layer has two phases. First, candidate labels are generated by shifting an existing node frontier by an arc weight. Second, candidates assigned to the same destination node are filtered by Pareto dominance.
...
The cutset layer introduces a fourth primitive. Once the root-side and terminal-side frontiers meet at layer $\mathcal{L}_c$, each cutset node $u$ contributes the product set
\[
