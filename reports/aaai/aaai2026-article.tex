...
We evaluate the proposed methods through three research questions. \textbf{RQ1} compares serial multiobjective decision diagrams (\textsc{Smdd}) with the MIP-based defining-point algorithm (DPA) in terms of runtime and robustness. \textbf{RQ2} measures the speedup obtained by the CPU variant of Parallel Multiobjective Decision Diagrams (\textsc{Pmdd}), denoted by \textsc{PmddC}, over \textsc{Smdd}. \textbf{RQ3} compares the GPU variant, denoted by \textsc{PmddG}, with \textsc{PmddC}, focusing on both runtime and the ability to solve instances within the prescribed resource limits.

\subsection{Computational Setup}

We conducted all experiments on a compute cluster equipped with an Intel Xeon 6972P CPU running at 2.4\,GHz and an NVIDIA H100 GPU. We imposed a one-hour time limit per instance, with memory limits of 16\,GB on the CPU and 20\,GB on the GPU. Our implementation builds on the C++ code of \citet{bergman2022network} and was compiled using C++14, OpenMP 4.5 for CPU parallelism, and NVCC 12.6 for GPU acceleration. We evaluate \textsc{PmddC} with 4, 8, and 16 threads, denoted by \textsc{PmddC}$(4)$, \textsc{PmddC}$(8)$, and \textsc{PmddC}$(16)$, respectively.

We consider the multiobjective knapsack problem (MOKP), multiobjective set packing problem (MOSPP), and multiobjective traveling salesperson problem (MOTSP), using the instances of \citet{bergman2022network}. Each problem setting contains 10 instances. We report settings for which the best serial bidirectional enumeration method of \citet{bergman2022network} required at least five seconds of total runtime.

We use the original implementation of \citet{bergman2022network} as the serial baseline and denote it by \textsc{Smdd}. This baseline applies their bidirectional enumeration algorithm \texttt{Coup}$^+$ to MOKP and \texttt{Coup} to MOSPP and MOTSP; the ``$+$'' indicates that state-dominance pruning is enabled. The corresponding \textsc{PmddC} and \textsc{PmddG} experiments use the same enumeration and pruning configuration as \textsc{Smdd}. We additionally report the GPU top-down variant as \texttt{G-TD}. For the MIP comparison, we use the DPA-A and DPA-TS variants of \citet{dachert2024simple}. DPA was shown to outperform the criterion-space methods against which \textsc{Smdd} was previously evaluated, but these two lines of work have not been compared directly. Our comparison therefore evaluates whether DD-based enumeration remains competitive with this stronger MIP baseline.

We report the arithmetic mean enumeration wall-clock time over the solved instances and the corresponding speedup relative to \textsc{Smdd}. The notation $S^{(M,T)}$ gives the number of solved instances $S$, with $M$ out-of-memory failures and $T$ timeouts; hence, $S+M+T=10$ for every problem setting. A dash indicates that no instance was solved.

\subsection{Results}

\paragraph{RQ1 (\textsc{Smdd} vs. DPA).}
\Cref{tab:combined_result_short} shows complementary performance regimes for DD- and MIP-based enumeration. \textsc{Smdd} is substantially more robust on MOKP and MOTSP: DPA solves only two MOKP instances for $(N,K)=(40,6)$ and none of the other reported MOKP settings, while both DPA variants time out on every reported MOTSP setting. In contrast, \textsc{Smdd} solves all MOKP instances with $N=40$ and all MOTSP instances with $K=4,5$. DPA is more effective on low-objective MOSPP settings, where DPA-A is faster than \textsc{Smdd} for $(150,3)$ and $(200,3)$ and solves all instances for $N=200$. This crossover reflects the different computational bottlenecks. DPA solves one MIP per nondominated point and therefore benefits when the final frontier is small, whereas DD enumeration is governed by the state-space representation and the sizes of the intermediate node frontiers. As the number of objectives increases, the growth of the final frontier makes DPA less robust, while \textsc{Smdd} continues to exploit shared state-space structure.

\paragraph{RQ2 (\textsc{Smdd} vs. \textsc{PmddC}).}
\textsc{PmddC} consistently improves upon \textsc{Smdd}, although the speedup depends on the problem family and thread count. The best CPU speedups on the reported MOKP settings range from $1.80\times$ to $3.21\times$, with weaker scaling for the larger $N=70$ instances. On MOSPP, the maximum speedup ranges from $1.51\times$ for $(150,3)$ to $4.12\times$ for $(200,4)$. MOTSP benefits most from CPU parallelism: \textsc{PmddC}$(16)$ attains speedups of $7.83\times$ and $5.05\times$ for $K=4$ and $K=5$, respectively. Increasing the thread count is not uniformly beneficial, as illustrated by MOKP with $(N,K)=(70,7)$ and MOSPP with $(150,7)$. This behavior is consistent with the variation in node-frontier workloads and the cost of merging the worker-local cutset frontiers. Nevertheless, \textsc{PmddC} also improves robustness on several difficult settings by solving more instances than \textsc{Smdd}.

\paragraph{RQ3 (\textsc{PmddC} vs. \textsc{PmddG}).}
\textsc{PmddG} provides the largest speedups when its intermediate frontiers fit in GPU memory. For MOKP with $N=40$, it attains speedups of $5.51\times$ and $9.71\times$, compared with at most $2.60\times$ and $3.21\times$ for \textsc{PmddC}. On MOSPP with $N=150$, \textsc{PmddG} reaches $2.44\times$ for $K=3$ and $10.98\times$ for $K=7$. It also outperforms the CPU configurations on the reported MOTSP settings, reaching $8.03\times$, $7.04\times$, $4.40\times$, and $1.94\times$ for $K=4,5,6,7$, respectively. The GPU is less robust when the DD and its intermediate frontiers exceed device memory: both GPU variants fail on the reported MOSPP instances with $N=200$, and memory failures also occur for larger MOKP and MOTSP settings. Candidate batching bounds the temporary storage used by an individual expansion, but the flattened DD and node frontiers must still reside in GPU memory. Thus, \textsc{PmddG} offers the strongest runtime performance on instances that fit in device memory, whereas \textsc{PmddC} degrades more gracefully as frontier sizes increase. Full results are provided in \Cref{app:additional_results}.

\begin{table*}[htbp!]
\centering
\scriptsize
\renewcommand{\arraystretch}{0.97}
\caption{Enumeration performance across problem families. Times are arithmetic means over solved instances, and speedups are relative to \textsc{Smdd}. Superscript ``$+$'' denotes the use of state-dominance pruning with \texttt{Coup}$^+$. Bold values indicate the fastest mean time for each setting. The entry $S^{(M,T)}$ reports solved instances, out-of-memory failures, and timeouts.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrlrrllrrlrrl}
\toprule
Problem & $N$ & $K$ & Method & Time (s) & Speedup & $S^{(M,T)}$ &
$N$ & $K$ & Method & Time (s) & Speedup & $S^{(M,T)}$ \\
\midrule
