...
\begin{abstract}
Decision diagrams (DDs) can enumerate the exact Pareto frontier (PF) for multiobjective discrete optimization problems that admit a dynamic programming formulation.
The Pareto dominance filtering at each node of the DD has a cost that grows quadratically in the size of the intermediate labels. Existing algorithms perform this filtering serially, making it the key bottleneck.
We parallelize dominance filtering along with layer expansion and the coupling and merging of node frontiers, developing implementations
tailored to both CPU and GPU.
...
Furthermore, our parallel enumeration variants achieve speedups of up to $7.83\times$ while GPU variants reach up to $20.88\times$ relative to serial enumeration, when intermediate frontiers fit in GPU memory.
...
Many discrete optimization problems that arise in finance, healthcare, routing, to name a few, are inherently multiobjective. In vehicle routing, for example, a dispatcher might need to balance the routing cost and service quality \citep{kovacs2015multi}. As these goals conflict, no single plan is best in terms of cost and service quality at once. To compare different plans we use the principle of \emph{Pareto dominance}, where one solution dominates another if it matches it on every objective and improves on at least one. The goal of exact multiobjective optimization~\citep{ehrgott2005multicriteria,ehrgott2016exact,miettinen1999nonlinear} is to enumerate every nondominated solution, since each one represents a distinct trade-off a decision-maker might reasonably prefer over the rest. The set of all nondominated solutions is known as the \emph{Pareto frontier} (PF). 
