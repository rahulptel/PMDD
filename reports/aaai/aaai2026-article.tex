...
% Our experimental setup and results are provided in \Cref{sec:setup} and \Cref{sec:results}, respectively. 
...
% Our work builds on the advances in the field of exact MODO and parallel discrete optimization. Exact multiobjective methods provide the DD-based framework used here to enumerate Pareto frontiers. Parallel discrete optimization studies how the computationally expensive operations of exact solvers can be distributed across multicore CPUs and GPUs. We review both areas below and explain how our approach connects them.
...
\subsection{Exact Multiobjective Discrete Optimization}
\label{sec:related_exact}
Exact MODO algorithms can be classified according to whether they organize the search in criterion space, decision space, or a DP state space \citep{ehrgott2016exact,halffmann2022exact}. Criterion-space methods partition the unexplored objective region and solve a sequence of scalarized or constrained single-objective subproblems. This class includes recursive and box-decomposition methods \citep{ozlen2009multi,ozlen2014multi,boland2015criterion}, parameter-space search \citep{kirlik2014new}, and more recent objective-space algorithms for multiobjective integer programming \citep{tamby2021enumeration,dachert2024simple}.

Decision-space methods retain a branch-and-bound or branch-and-cut tree over the original decision variables. Early algorithms adapted integer-programming enumeration to identify efficient solutions \citep{klein1982algorithm,kiziltan1983algorithm}. More recent biobjective methods replace scalar node bounds with vector-valued bound sets and compare them against the incumbent PF. Research in this area has developed stronger bound sets and fathoming tests, problem-independent branching rules, presolve techniques, and hybrid strategies that also branch in objective space \citep{stidsen2014branch,parragh2019branch,adelgren2022branch}. These methods avoid an explicit partition of the entire objective space, but their effectiveness depends on the strength and computational cost of the bound sets computed throughout the search tree.

State-space methods use a DP network rather than a branch-and-bound tree. The DD framework used in this paper follows \citet{bergman2016multiobjective,bergman2022network} and is summarized in \Cref{sec:prelim_dd}. Our contribution leaves the underlying exact DD model unchanged and parallelizes the operations used to enumerate its PF. DDs have also been used in constraint programming for a different purpose. The Pareto constraint maintains an archive of
nondominated solutions found during tree search and uses it to filter the objective-variable domains \citep{hartert2014pareto}.
\citet{malalel2023mdd} encode this archive as an MDD to accelerate its update and filtering operations. In our setting, the DD
represents the DP state space, and its node frontiers contain nondominated vectors associated with partial paths. The archive is
therefore updated incrementally during constraint-programming search, whereas our method propagates labels over a fixed state-
space DD to recover the PF.


\subsection{Parallel Computing for Discrete Optimization}

Parallel exact optimization has traditionally focused on branch-and-bound, where nodes or subtrees are distributed across workers \citep{kindervater1986introduction,gendron1994parallel,ralphs2003parallel,linderoth2003decomposition}. Its performance depends on balancing the search workload while limiting communication and synchronization. Parallel execution can also change the order in which incumbents and bounds are discovered, sometimes increasing the total amount of search \citep{lai1984anomalies}. Parallelism has also been explored within DD-based optimization. \citet{bergman2014parallel} distribute subproblems generated by an exact branch-and-bound framework based on approximate DDs, while \citet{perez2018parallel} parallelize reduction and set operations that manipulate the structure of an MDD. In contrast, \textsc{PmddC} operates on an already constructed exact DD. It distributes node-frontier expansion across CPU threads and combines the resulting cutset frontiers through a parallel reduction.


GPUs have been used to accelerate simplex and interior-point computations \citep{lalami2011gpu,ploskas2016gpu,shin2025madipm}, and GPU-native first-order methods have shown strong performance on large linear programs \citep{applegate2021practical,lu2025cupdlp,lu2023cupdlpc,lu2025cupdlpx}. GPU acceleration is more difficult for mixed-integer optimization because branch-and-bound produces irregular and dynamically changing workloads. Existing work has therefore focused largely on selected solver components and primal heuristics \citep{boyer2017gpu,ccorduk2025gpu}. More closely related to our work, \citet{tardivo2026gpu} accelerate state expansion during anytime exact DD search, including successor generation, state dominance filtering, and duplicate elimination. Their method uses the GPU while constructing and exploring DD fragments. \textsc{PmddG}, by contrast, assumes that the exact DD has already been constructed and parallelizes the subsequent PF enumeration. It flattens the DD and its node frontiers into arrays that support batched candidate generation, dominance filtering, and frontier reduction. Thus, both methods exploit the layered structure of DDs, but they parallelize different stages and process different objects. \citet{tardivo2026gpu} process DP states during DD construction and search, whereas \textsc{PmddG} processes objective labels during PF
enumeration.

% Parallel exact optimization has historically focused on branch-and-bound. Nodes or subtrees can be assigned to different workers, allowing large portions of the search tree to be explored concurrently \citep{kindervater1986introduction,gendron1994parallel,ralphs2003parallel,linderoth2003decomposition}. The effectiveness of this strategy depends on balancing the search workload while limiting communication and synchronization. Parallel execution can also change the order in which incumbents and bounds are discovered, sometimes increasing the total amount of search required \citep{lai1984anomalies}.

% DDs have enabled several other forms of parallelism. \citet{bergman2014parallel} distribute the subproblems of an exact branch-and-bound search based on recursively constructed approximate DDs. \citet{perez2018parallel} instead parallelize reduction, union, intersection, and difference operations that manipulate the structure of an MDD. More recently, \citet{tardivo2026gpu} use the GPU to accelerate state expansion during exact anytime DD search, including successor generation, dominance filtering, and duplicate elimination. These approaches parallelize the search process, operations on the diagram, or the construction of the diagram itself. Our method assumes that an exact DD is already available and parallelizes the label propagation, dominance filtering, and cutset coupling required to enumerate its PF.

% GPUs have also been applied more broadly in mathematical optimization. Prior work has accelerated simplex and interior-point computations \citep{lalami2011gpu,ploskas2016gpu,shin2025madipm}, while GPU-native first-order methods have enabled efficient solution of large linear programs \citep{applegate2021practical,lu2025cupdlp,lu2023cupdlpc,lu2025cupdlpx}. Extending this success to mixed-integer optimization is more difficult because branch-and-bound produces irregular and dynamically changing workloads. Existing work has therefore focused largely on selected solver components and primal heuristics \citep{boyer2017gpu,ccorduk2025gpu}. In DD-based PF enumeration, the labels generated within each layer can be flattened into large batches of objective vectors. This exposes fine-grained parallelism in candidate generation, dominance testing, and frontier reduction without transferring search-tree control to the GPU.
...
This paper introduced \textsc{Pmdd}, parallel CPU and GPU implementations of exact bidirectional PF enumeration on DDs. \textsc{PmddC} distributes node-frontier expansion across CPU threads and combines cutset frontiers through a parallel reduction, while \textsc{PmddG} parallelizes candidate generation, dominance filtering, and frontier reduction at a finer granularity. Experiments on MOKP, MOSPP, and MOTSP showed that DD-based enumeration is more robust than the MIP baseline on MOKP and MOTSP, while the MIP approach remains competitive on large, low-objective MOSPP instances. 
% Relative to \textsc{Smdd}, the CPU and GPU variants achieved speedups of up to $7.83\times$ and $20.88\times$, respectively. 
The GPU provides the strongest performance when the DD and intermediate frontiers fit in device memory, whereas the CPU remains more reliable under memory pressure.

Future work will address the remaining scalability limits on both architectures. On the CPU, dividing large node-frontier updates among multiple threads could reduce load imbalance and improve scaling beyond the current node-level decomposition. On the GPU, eliminating duplicate top-down and bottom-up arc storage, reusing temporary buffers more aggressively, and routing operations that exceed device memory to the CPU could improve robustness without sacrificing GPU performance on smaller frontiers. Developing a DD-based branch-and-bound framework for solving MODO problems can also help in addressing the memory pressure as one could avoid constructing and retaining an exact DD in memory, along with the corresponding node frontiers. Note that the parallelization techniques developed in this work can be directly applied to such a framework to accelerate the recovery of primal and dual bounds sets from restricted and relaxed DDs, respectively.
...
\toprule
$n$ & $m$ & Method & Time (S) & Speed-up & $S^{(M,T)}$ & $n$ & $m$ & Method & Time (S) & Speed-up & $S^{(M,T)}$ \\
\midrule
