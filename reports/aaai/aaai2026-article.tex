...
\begin{abstract}
Decision diagrams (DDs) are an effective exact framework for multiobjective discrete optimization because they enable Pareto frontier (PF) enumeration over a compact dynamic-programming representation of the feasible set. Existing DD-based PF enumeration algorithms, however, are serial, even though their dominant operations expose substantial parallelism. This paper presents parallel CPU and GPU implementations of exact DD-based PF enumeration. The proposed implementations preserve the bidirectional enumeration model while parallelizing layer expansion, candidate-label generation, dominance filtering, cutset-product materialization, and frontier merging. We evaluate the methods on benchmark instances of the multiobjective knapsack, set packing, and traveling salesperson problems with up to seven objectives, comparing against both a state-of-the-art MIP approach and the state-of-the-art serial DD implementation. DD-based enumeration is more robust than the MIP baseline on MOKP and MOTSP, while the MIP baseline remains competitive on some lower-objective MOSPP instances. Relative to serial DD enumeration, the parallel CPU variants achieve speedups of up to $7.83\times$, and the GPU variants achieve speedups of up to $20.88\times$ when intermediate frontiers fit in GPU memory.
The code is available at \url{https://anonymous.4open.science/r/PDD}.
\end{abstract}


\section{Introduction}
Multiobjective discrete optimization (MODO) \cite{ehrgott2005multicriteria,ehrgott2016exact,miettinen1999nonlinear} problems arise when feasible decisions must be evaluated under several conflicting criteria. Although problem-specific heuristics \cite{Vianna2014AMG} and evolutionary methods \cite{Deb2002AFA} are widely used, exact enumeration of the Pareto frontier (PF) remains important when all trade-offs must be certified. Decision diagrams (DDs) \cite{Cir2018DecisionDF} provide an effective exact framework for discrete problems with a dynamic-programming structure. An exact DD compactly represents the feasible set as a layered acyclic graph; PF enumeration can then be performed by propagating nondominated labels through this graph. Recent DD-based methods have shown strong performance on MODO instances with several objectives and tens to hundreds of decision variables \cite{bergman2022network}.
