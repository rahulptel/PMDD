...
The CPU implementation parallelizes the node loops in Algorithm~\ref{alg:coupled_pf}. For root-side expansion, this is the loop in line~5; for terminal-side expansion, it is the loop in line~14. The loop is scheduled dynamically because the number of candidate labels can vary substantially across nodes in the same layer.
...
The GPU implementation follows the same enumeration logic as Algorithm~\ref{alg:coupled_pf}, but changes the unit of work. Rather than assigning one worker to own a node frontier, it materializes candidate labels in flat buffers and then applies data-parallel filtering and compaction kernels. The high-level procedure is shown in \Cref{alg:gpu_coupled_pf}: the DD is first flattened, each selected layer expansion invokes the same four-kernel pipeline, and the cutset phase is handled by a separate batched coupling routine.
...
For a selected layer, \Cref{alg:gpu_coupled_pf} calls the four subroutines in \Cref{alg:compute_arc_offsets,alg:expand_candidates,alg:dynamic_1D_kernel,alg:compact_survivors}. The dataflow is
\[
    \mathcal{Y}
...
We evaluate the proposed methods on MOKP, MOSPP, and MOTSP. The test instances are taken from \citet{bergman2022network}. We report results for instance sizes on which their best method, namely coupled enumeration (\texttt{Coup} or \texttt{Coup}$^+$), required at least 5 seconds of total runtime. For each instance size, the test set contains 10 instances.

We compare our approach against the serial coupled enumeration method of \citet{bergman2022network}, denoted by \text{Coup} or \text{Coup}$^+$. The ``+'' suffix indicates that state-dominance-based label pruning is used in addition to coupled enumeration. To ensure a fair comparison, we use the implementation provided by \citet{bergman2022network}, rather than our modified implementation, which can also perform serial PF enumeration. We further compare against the defining point algorithm (DPA) proposed in \citet{dachert2024simple}, which, to the best of our knowledge, is a state-of-the-art MIP-based approach for multiobjective discrete optimization with support for more than two objectives. We refer to the two variants of this algorithm as DPA-A and DPA-TS.

Our CPU-based parallel variants are referred to as \texttt{Coup-T} (or $\texttt{Coup}^+\texttt{-T}$), where \texttt{T} specifies the number of parallel threads. The GPU variants are called $\texttt{G-Coup}$ or $\texttt{G-Coup}^{+}$. he corresponding GPU variants are denoted by \texttt{G-Coup} and \texttt{G-Coup}$^+$. We report the enumeration wall-clock time in seconds (``Time (s)''), the speed-up w.r.t. the Coup or Coup+ and the tuple ``(M, T)'' reports the number of instances that failed due to out-of-memory errors (M) and time-out limits (T).

\subsection{Results}
\paragraph{RQ1 (MIP vs. Serial DD).}
\Cref{tab:mokp_result_short,tab:motsp_result_short} show that serial DD enumeration is substantially more robust than the DPA baselines on MOKP and MOTSP. On MOKP, DPA solves only two instances for $(N,K)=(40,6)$ and none of the remaining reported settings, whereas \texttt{Coup} solves all instances for $N=40$ and remains competitive on the larger $N=70$ settings. On MOTSP, both DPA variants time out on all reported settings, while \texttt{Coup} solves all instances for $K=4,5$, nine instances for $K=6$, and one instance for $K=7$. The MOSPP results in \Cref{tab:mospp_result_short} are more mixed: DPA-A is faster than \texttt{Coup} for $(150,3)$ and $(200,3)$, and solves all instances where \texttt{Coup} encounters memory failures for $N=200$; however, DPA fails on the high-objective $(150,7)$ setting where \texttt{Coup} solves all instances.
...
N & K & Method & Time (S) & Speed-up & $S^{(M,T)}$ & N & K & Method & Time (S) & Speed-up & $S^{(M,T)}$ \\
\midrule
\multirow{8}{*}{150} & \multirow{8}{*}{3} & DPA-TS & 11 & 1.12$\times$ & $10$ & \multirow{8}{*}{150} & \multirow{8}{*}{7} & DPA-TS & -- & -- & $0^{(0, 10)}$ \\
...
% For instance, in the coupled enumeration scheme (Algorithm~\ref{alg:coupled_pf}), the execution of \textsc{ExpandTopDown} (lines 5--10) and \textsc{ExpandBottomUp} (lines 14--19) can be parallelized over the outer loop of destination nodes. Furthermore, at the cutset layer $\mathcal{L}_c$, the convolution of the top-down and bottom-up frontiers (lines 34--36) can be parallelized across individual nodes since each convolution operates strictly on local label sets $\mathcal{Y}^{\downarrow}(u)$ and $\mathcal{Y}^{\uparrow}(u)$ associated with node $u$. By mapping a single thread per node, this scheme circumvents data races without the need for expensive thread synchronization mechanisms like locks.
